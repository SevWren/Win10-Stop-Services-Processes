...
#AutoIt3Wrapper_Outfile=g:\Users\Administrator\Desktop\Stop Services - Processes.Exe
...
Func _stopservicescustom() ; Check if a service is running and stop it, then log the closure
	...
	If _ServiceRunning("", "sppsvc") Then
		...
		If $iResult2 = 1 Then
			$iSuccess += 1
			_LogMessage("Successfully stopped sppsvc process" & @CRLF, "Service", 1)
		Else
			$iFailure += 1
			_LogMessage("Failed to stop sppsvc process (Error: " & @error & ")" & @CRLF, "Service", 0)
		EndIf
	EndIf
...
Func _exit()
	...
	_LogMessage("Script Was Terminated by _exit()", "System" & @CRLF, 1)
	Exit
EndFunc   ;==>_exit

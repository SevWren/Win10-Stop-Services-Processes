...
; Set the tray icon using the specified file
TraySetIcon($iconfile)

; Initialize logging: Create the log directory if it doesn’t exist
...
; Function: Toggle the script’s active state with F1 key
Func ToggleScript()
    ...
    ; Inner loop: Executes while the script is active
    While $bScriptRunning
        ; Check processes every 2 seconds
        If CheckElapsedTime($iLastStopProcesses, 2) Then
            _CloseInstaller()
            $iLastStopProcesses = TimerInit()
        EndIf
        ; Check services every 2 seconds
        If CheckElapsedTime($iLastStopServices, 2) Then
            _stopservicescustom()
...
; Function: Close specified processes listed in $sProcesses
...
; Function: Stop specified services and handle sppsvc process separately
Func _stopservicescustom()
    ...
    ; Attempt to stop each service if it’s running
    ...
    ; Special handling for sppsvc: Close its process if the service is active
...
; Function: Log messages to a file and optionally to the console
...
; Function: Handle script termination with logging
Func _exit()
...
; Function: Close a specific window if it exists (likely for "Advanced Renamer")
...
; Utility Function: Check if a specified time interval (in seconds) has elapsed
Func CheckElapsedTime($iStartTime, $iInterval)

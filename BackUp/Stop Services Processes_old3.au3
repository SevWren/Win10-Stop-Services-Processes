...
HotKeySet("^9", "SetProcessPriority") ;ctrl and 9 hotkey for process priority
...
Func SetProcessPriority()
    Local $sProcessName = InputBox("Process Priority", "Enter process name:", "", "*", -1, -1, 0, 0, "", "", "")
    ...
    Local $sPriorityLevel = InputBox("Process Priority", "Select priority level:" & @CRLF & "1 - Below Normal" & @CRLF & "2 - Normal" & @CRLF & "3 - Above Normal", "", "*", -1, -1, 0, 0, "", "", "")
    If @error Then ; User pressed Cancel
    ...
    If ProcessExists($sProcessName) Then
        If ProcessSetPriority($sProcessName, $iPriorityConst) Then
            Local $sPriorityName
            ...
            EndSwitch
            _LogMessage("Set process '" & $sProcessName & "' priority to " & $sPriorityName, "ProcessPriority", 1)
            MsgBox($MB_SYSTEMMODAL, "Process Priority", "Set process '" & $sProcessName & "' priority to " & $sPriorityName, 1)
        Else
            _LogMessage("Failed to set process '" & $sProcessName & "' priority (Error: " & @error & ")", "ProcessPriority", 0)
            MsgBox($MB_SYSTEMMODAL, "Process Priority", "Failed to set process '" & $sProcessName & "' priority (Error: " & @error & ")", 1)
        EndIf
    Else
        _LogMessage("Process '" & $sProcessName & "' is not running.", "ProcessPriority", 1)
        MsgBox($MB_SYSTEMMODAL, "Process Priority", "Process '" & $sProcessName & "' is not running.", 1)
    EndIf

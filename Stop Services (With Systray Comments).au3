...
;~ Global $logdir = "G:\login"
;~ Global $sLogFile = "G:\login\closure_log.txt" ;LogFileForMikeDesk
Global $sLogFile = "C:\Users\User\Desktop\closure_log.txt" ;LogFileForServer
...
#cs Func _stopservicescustom()
	...
	For $i = 0 To UBound($aServiceNames) - 1
		...
		If _ServiceRunning("", $aServiceNames[$i]) Then
			...
			$bServiceStopped = True
			; Log the service closure
			LogServiceClosure($aServiceNames[$i])
		EndIf
	Next
	...
	If _ServiceRunning("", "sppsvc") Then
		...
		$bServiceStopped = True
		; Log the service closure
		LogServiceClosure("sppsvc")
	EndIf
EndFunc   ;==>_stopservicescustom
#ce Func _stopservicescustom()
#cs Func _stopservicescustom() ;run cmd prompt and pass net stop commands and set cmd prompt to hidden
	Local $bServiceStopped = False
	; Define an array of service names
	Local $aServiceNames = ["TrustedInstaller", "wuauserv", "UsoSvc", "DoSvc", "WaaSMedicSvc", "sppsvc"]
	For $i = 0 To UBound($aServiceNames) - 1      ; Loop through each service name in the array
		; Check if the service is running
		If _ServiceRunning("", $aServiceNames[$i]) Then
			; Stop the service
			_stopservice("", $aServiceNames[$i])
		...
		EndIf
	Next
	If _ServiceRunning("", "sppsvc") Then ; Special handling for sppsvc service as it requires ProcessClose
		ProcessClose("sppsvc.exe")
		$bServiceStopped = True
		LogServiceClosure("sppsvc")
	EndIf
#cs
		If _ServiceRunning("", "TrustedInstaller") Then
		...
		If _ServiceRunning("", "wuauserv") Then
			...
			$bServiceStopped = True
		EndIf
		If _ServiceRunning("", "UsoSvc") Then
			_stopservice("", "UsoSvc") ;stop update orchestrator service
			$bServiceStopped = True
		EndIf
		If _ServiceRunning("", "DoSvc") Then
			_stopservice("", "DoSvc") ;stop delivery optimization service
			$bServiceStopped = True
		EndIf
		If _ServiceRunning("", "WaaSMedicSvc") Then
			_stopservice("", "WaaSMedicSvc") ;stop windows update medic service
			$bServiceStopped = True
		EndIf
		If _ServiceRunning("", "sppsvc") Then
			ProcessClose("sppsvc.exe")
...
#ce Func _stopservicescustom() ;run cmd prompt and pass net stop commands and set cmd prompt to hidden
#cs Func  LogServiceClosure($sServiceName); Function to log the date and time of service closure
	; Get the current date and time
	ConsoleWrite("In LogServiceClose" & @CR)
	Local $sDateTime = _NowTime(12) & " " & @MDAY & "/" & @MON & "/" & @YEAR
	; Create the log entry
	Local $sLogEntry = $sDateTime & " - Service '" & $sServiceName & "' was closed." & @CRLF
	; Define the log file path
	Local $sLogFile = $logdir & "\StopServices_log.txt"
	; Write the log entry to the log file
	FileWrite($sLogFile, $sLogEntry)
EndFunc   ;==>LogServiceClosure
#ce Func  LogServiceClosure($sServiceName); Function to log the date and time of service closure
